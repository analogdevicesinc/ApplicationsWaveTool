...
enum M2M2_BIA_APP_CMD_ENUM_t:uint8_t {
  ...
  M2M2_BIA_APP_CMD_READ_ALGO_PARAMETERS_REQ = 85,
  M2M2_BIA_APP_CMD_READ_ALGO_PARAMETERS_RESP = 86,
};
...
struct bia_app_read_algo_parameters_req_t {
  uint8_t  command; 
  uint8_t  status; 
};

struct bia_app_read_algo_parameters_resp_t {
  uint8_t  command; 
  uint8_t  status; 
  float  ffm_estimated[10]; 
  float  fat_percent[10]; 
  float  bmi[10]; 
  uint32_t  time_stamp; 
};
